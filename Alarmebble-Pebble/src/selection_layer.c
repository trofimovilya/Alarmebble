...
#define DEFAULT_ACTIVE_COLOR GColorDarkGray
...
static int prv_get_pixels_for_bump_settle(int anim_percent_complete) {
  if (anim_percent_complete) {
    return SETTLE_HEIGHT_DIFF -
           ((SETTLE_HEIGHT_DIFF * anim_percent_complete) / 100);
  } else {
    return 0;
  }
}

static int prv_get_font_top_padding(GFont font) { return 10; }

static int prv_get_y_offset_which_vertically_centers_font(GFont font,
                                                          int height) {
  int font_height = 14;
  int font_top_padding = 10;
...
static void prv_draw_cell_backgrounds(Layer *layer, GContext *ctx) {
  ...
  for (int i = 0, current_x_offset = 0; i < data->num_cells; i++) {
    ...
    if (data->selected_cell_idx == i && data->bump_is_upwards) {
      y_offset =
          -prv_get_pixels_for_bump_settle(data->bump_settle_anim_progress);
    }
...
static void prv_draw_slider_slide(Layer *layer, GContext *ctx) {
  ...
  int current_slide_distance =
      (slide_distance * data->slide_amin_progress) / 100;
  if (!data->slide_is_forward) {
  ...
  int total_cell_width_change =
      next_cell_width - cur_cell_width + data->cell_padding;
  ...
  GRect rect = GRect(current_x_offset, 0, current_cell_width,
                     layer_get_bounds(layer).size.h);
...
static void prv_draw_slider_settle(Layer *layer, GContext *ctx) {
  ...
  GRect rect =
      GRect(x_offset, 0, current_width, layer_get_bounds(layer).size.h);
      ...
      if (text) {
        ...
        if (data->selected_cell_idx == i) {
          int delta = (data->bump_text_anim_progress *
                       prv_get_font_top_padding(data->font)) /
                      100;
        ...
        GRect rect =
            GRect(current_x_offset, y_offset, data->cell_widths[i], height);
        graphics_draw_text(ctx, text, data->font, rect, GTextOverflowModeFill,
...
static void prv_bump_text_impl(struct Animation *animation,
  ...
  data->bump_text_anim_progress =
      (100 * distance_normalized) / ANIMATION_NORMALIZED_MAX;
  layer_mark_dirty(layer);
}

static void prv_bump_text_stopped(Animation *animation, bool finished,
                                  void *context) {
  Layer *layer = (Layer *)animation_get_context(animation);
  SelectionLayerData *data = layer_get_data(layer);
...
static void prv_bump_settle_stopped(Animation *animation, bool finished,
                                    void *context) {
  Layer *layer = (Layer *)animation_get_context(animation);
  SelectionLayerData *data = layer_get_data(layer);
...
static void prv_slide_settle_stopped(Animation *animation, bool finished,
                                     void *context) {
  Layer *layer = (Layer *)animation_get_context(animation);
  SelectionLayerData *data = layer_get_data(layer);
...
void prv_up_click_handler(ClickRecognizerRef recognizer, void *context) {
  ...
  if (data->is_active) {
    if (click_recognizer_is_repeating(recognizer)) {
      ...
      data->callbacks.increment(data->selected_cell_idx,
...
void prv_down_click_handler(ClickRecognizerRef recognizer, void *context) {
  ...
  if (data->is_active) {
    if (click_recognizer_is_repeating(recognizer)) {
      // Don't animate if the button is being held down. Just update the text
      data->callbacks.decrement(data->selected_cell_idx,
                                click_number_of_clicks_counted(recognizer),
                                data->context);
      layer_mark_dirty(layer);
    } else {
...
void prv_select_long_click_handler(ClickRecognizerRef recognizer,
                                   void *context) {
  Layer *layer = (Layer *)context;
  SelectionLayerData *data = layer_get_data(layer);
  data->callbacks.change_mode(data->context);
}
...
static Layer *selection_layer_init(SelectionLayerData *selection_layer_,
                                   GRect frame, int num_cells) {
...
static void selection_layer_deinit(Layer *layer) { layer_destroy(layer); }
...
void selection_layer_set_click_config_onto_window(Layer *layer,
  ...
  if (layer && window) {
    window_set_click_config_provider_with_context(
        window, (ClickConfigProvider)prv_click_config_provider, layer);
  }
}

void selection_layer_set_callbacks(Layer *layer, void *context,
